...
- [ ] **Daemon-mode server** (`lmp server start|stop|status`). Eliminates per-invocation cold-start. Only worth doing if the per-invocation UX hurts in practice.
...
- [ ] **CWD independence.** Resolve `prompts/`, `profiles/`, `outputs/` relative to either the package install location or a `--repo-root` option, so `lmp` can run from anywhere.
